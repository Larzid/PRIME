...
shMapLevel::isRadioactive (int x, int y)
{
    ...
    for (int tx = maxi (0, x-1); tx <= mini (MAPMAXCOLUMNS-1, x+1); ++tx) {
        for (int ty = maxi (0, y-1); ty <= mini (MAPMAXROWS-1, y+1); ++ty) {
            ...
            if (f and f->mType == shFeature::kRadTrap)
                ++n;
...
shMapLevel::reset ()
{
    ...
    memset (&mFeatures, 0, sizeof (mFeatures));
...
shMapLevel::removeFeature (shFeature *f)
{
    if (f->isDoor ())
        doorRemoved (f);
    mFtList.remove (f);
    mFeatures[f->mX][f->mY] = NULL;
    delete f;
}


void
shMapLevel::putFeature (shFeature *f)
{
    mFtList.add (f);
    mFeatures[f->mX][f->mY] = f;
}
