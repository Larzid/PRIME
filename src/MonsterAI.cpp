...
shCreature::doGuard ()
{
    ...
    enum {GD_UNCHALLENGED, GD_CHALLENGED, GD_JANITOR, GD_NINJA, GD_PLAIN_NINJA};
    ...
    if (!canSee (hero)) {
        ...
        if (mGuard.mToll > 0 and mGuard.mChallengeIssued == GD_CHALLENGED) {
            mGuard.mChallengeIssued = GD_UNCHALLENGED;
        }
        if (   mGuard.mChallengeIssued == GD_NINJA
            or mGuard.mChallengeIssued == GD_PLAIN_NINJA)
        {
            mGuard.mChallengeIssued = GD_UNCHALLENGED;
        }
        return elapsed;
    ...
    if (looks_like (hero, dress::Janitor)) {
    ...
    if (looks_like (hero, dress::Ninja)) {
        switch (mGuard.mChallengeIssued) {
        case GD_NINJA:
        case GD_PLAIN_NINJA:
            break;
        ...
        case GD_UNCHALLENGED:
        default:
            mGuard.mChallengeIssued = GD_NINJA;
            if (Hero.cr ()->tryToTranslate (this)) {
                I->p ("\"Halt!  Rogue agents not allowed to enter the Robot Town burbclave!\"");
            } else {
                I->p ("%s beeps and screeches intimidatingly.", the_guard);
            }
        }
    }
        ...
        hero->mY >= mGuard.mSY and hero->mY <= mGuard.mEY)
    {
        ...
        case GD_JANITOR:
            ...
            break;
        case GD_CHALLENGED: case GD_NINJA: case GD_PLAIN_NINJA: default:
            getangry = true;
    ...
    /* Hero is in view. */
    switch (mGuard.mChallengeIssued) {
    case GD_CHALLENGED: case GD_PLAIN_NINJA:
        return elapsed;
    case GD_JANITOR:
        if (hero->tryToTranslate (this)) {
    ...
    case GD_NINJA:
        if (!looks_like (hero, dress::Ninja)) {
            mGuard.mChallengeIssued = GD_PLAIN_NINJA;
            if (hero->tryToTranslate (this)) {
                I->p ("\"I don't buy it, punk!  You just changed in front of me.\"");
            } else {
                I->p ("%s clicks and whirrs in exasperation.", the_guard);
            }
        }
        return elapsed;
    case GD_UNCHALLENGED:
