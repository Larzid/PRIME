...
shTextViewer::shTextViewer (const char *fname, const char *fpath, const char *(*filter)(const char *))
{   /* Try opening target file. */
    assert (fname);
    if (fpath == NULL)  fpath = HELPDIR;
    mFree = true;
    mLines = NULL;
    mNumLines = 0;
    mMaxWidth = 0;
    char path[PRIME_PATH_LENGTH];
    snprintf (path, sizeof (path)-1, "%s/%s", fpath, fname);
    FILE *text = fopen (path, "r");
    if (!text) return;
    /* Count lines and check max width. */
    char *buf = GetBuf ();
    while (fgets (buf, SHBUFLEN, text)) {
        const char *procbuf = buf;
        if (filter)  procbuf = (*filter)(buf);
        mMaxWidth = maxi (mMaxWidth, strlen (procbuf));
        ++mNumLines;
    }
    ...
    rewind (text);
    int line = 0;
    while (fgets (buf, mMaxWidth, text)) {
        const char *procbuf = buf;
        if (filter)  procbuf = (*filter)(buf);
        strcpy (mLines + line * mMaxWidth, procbuf);
        ++line;
    }
...
shMenu::accumulateResults ()
{
    ...
    while (1) { /* Menu loop. */
        ...
        for (i = mOffset; i < mLast; ++i) {
            ...
            } else if (item->mLetter >= 0 and (kCategorizeObjects & mFlags)
                and ((shObject *) item->mValue.mPtr)->is (obj::unpaid))
            {
                I->setWinColor (menuwin, kCyan, kBlack);
            }
