...
void
shCrtInterface::cursorOnXY (int x, int y, int)
...
void
shCrtInterface::refreshWin (Window)
{
    pio_refresh ();
}
...
shCrtInterface::showVersion ()
{
    ...
    pio_textattr (kBrown);
    pio_string (10 + 20, 7 + 3, PRIME_VERSION"/Astral Navigator");
...
void
shCrtInterface::setWinColor (Window, shColor fg, shColor bg)
{
...
void
shCrtInterface::draw (int x, int y, shCreature *c, shObjectIlk *oi, shObject *o, shFeature *f, shSquare *s)
{
...
void
shCrtInterface::drawMem (int x, int y, shCreature *c, shObjectIlk *oi, shFeature *f, shSquare *s)
{
    eff::type e = Level->mEffects[x][y];
...
void
shCrtInterface::drawEffect (int x, int y, eff::type e)
{
    shGlyph g = getASCII (x, y, e, NULL, NULL, NULL, NULL, NULL);
    if (g.mSym == 0)
        return;

    /* TODO: background changing effects are unimplemented. */
