...
const char *shObject::your (int cnt) {
    return stackDesc (
        cnt,
        is (obj::unpaid) ? "clerkbot's " : "your ",
        getShortDescription (cnt));
}
...
shObject::isClicking ()
{
    if (!isA (kObjGeigerCounter) or (isBuggy () and !NOT0 (mOwner,->isOrc ())))
        return false;
    return Hero.getStoryFlag ("geiger counter message") > 0;
}
