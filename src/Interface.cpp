...
shInterface::readKeybindings (const char *fname, int n_keys, const shNameToKey KeyNames[])
{
    ...
    snprintf (keymapname, sizeof (keymapname)-1, "%s/%s", KEYMAPDIR, fname);
...
shInterface::drawSideWin (shCreature *c)
{
    ...
    if (c->isInShop ()) {
        ...
        winPrint (kSide, 12, 8, "$%d", c->countDebt ());
    }
    ...
    /* Ammunition display. */
    if (!c->isInShop () && weapon and (weapon->isA (kWeapon) or weapon->isA (kRayGun))) {
