...
struct shVector
{
 public:
    ...
    void ensureWithInit (int capacity);
    void setCount (int size);

 protected:
    void grow (int newCapacity);
    void growAndInit (int newCapacity);
...
template <class T>
struct shHeap : public shVector <T>  /* T must be a pointer type */
{
public:
...
template <class T>
void
shVector<T>::ensureWithInit (int capacity)
{
    if (mCapacity < capacity)
        growAndInit (capacity);
}
...
template <class T>
void
shVector<T>::growAndInit (int newCapacity)
{
    size_t before = sizeof (mItems);
    grow (newCapacity);
    size_t after = sizeof (mItems);
    memset ((char *)mItems + before, 0, after - before);
}
