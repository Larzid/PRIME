...
static void
safe_vect_set (shVector<void *> *ptrvect, int position, void *value)
{
    if (position >= ptrvect->count ()) {

        ptrvect->ensure (position);

        for (int i = ptrvect->count (); i < position; ++i)
            ptrvect->set (i, NULL);
    }
    ptrvect->set (position, value);
}

static void
safe_vect_set1 (shVector <UpdatePair *> *ptrvect, int position, UpdatePair *value)
{
    if (position >= ptrvect->count ()) {
...
loadPointer (int fd, void **ptr, const char *msg)
{
    ...
    *ptr = NULL;
    ...
    if (id < UpdateList.count ()) {
        UpdatePair *p = UpdateList.get (id);
        if (p != NULL) {
            p->mTargets.add (ptr);
            return;
        }
    }
    UpdatePair *p = new UpdatePair (id);
    p->mTargets.add (ptr);
    safe_vect_set1 (&UpdateList, id, p);
}
...
loadStruct (int fd, void *ptr)
{
    ...
    if (id < UpdateList.count ()) {
        ...
        for (int i = 0; i < p->mTargets.count (); ++i) {
            * p->mTargets.get (i) = ptr;
            debug.log ("                %p <- %p", ptr, p->mTargets.get (i));
        }
        UpdateList.set (id, NULL);
        delete p;
        return;
    }
}
...
shMapLevel::saveState (int fd)
{
    ...
    i = mFtList.count ();
    saveInt (fd, i);
    for (i = 0; i < mFtList.count (); i++) {
        SWRITE (fd, mFtList.get (i), sizeof (shFeature));
    }
    saveMagic (fd, MAPEND);
}
...
loadGame (const char* name)
{
    ...
    snprintf (savename, sizeof(savename)-1, "%s/save/%s.sav", UserDir, name);
    ...
    for (int i = 0; consistency and i < UpdateList.count (); ++i)
        consistency = UpdateList.get (i) == NULL;

    if (!consistency) {
        int giveup = 1;

        if (BOFH) {
            giveup = I->yn ("Some pointers uninitialized, abort?");
        } else {
        ...
        for (int i = 0; i < UpdateList.count (); i++)
            if (UpdateList.get (i) != NULL)
                debug.log ("Still need object %d", i);
    ...
    if (!BOFH or I->yn ("erase save file?"))
        unlink (savename);
