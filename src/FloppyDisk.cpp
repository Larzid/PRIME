...
static int
programWriteChance (shObjId id, shObject *comp, shObject *disk, bool estimate)
{
...
doBlank (shObject *computer, shObject *disk)
{
    ...
    if (Hero.cr ()->is (kConfused)) {
        ...
        do
        {
            ...
            if (strptr) {
                ...
                int dc = programWriteChance (shObjId (i), computer, disk, true);
    ...
    if (chance == -1)  /* Check hero's skill. */
        chance = programWriteChance (progid, computer, disk, false);
...
doDetectObject (shObject *computer, shObject *disk)
{
    ...
    if (!Hero.cr ()->is (kConfused)) {
        ...
        for (int i = 0; i < Level->mFtList.count (); ++i) {
            shFeature *f = Level->mFtList.get (i);
            if (!disk->is (obj::infected) or RNG (3)) {
...
doDetectBugs (shObject *computer, shObject *disk)
{
    ...
    /* TODO: For infected disks sum of buggy items found and buggy items
       ...
       automatically by player character based on Intelligence. */
    return kNormal;
}
...
doWarpTrace (shObject *, shObject *disk)
{
    ...
    shMenu *trace_menu;
    if (disk->isOptimized ()) {
        trace_menu = I->newMenu ("Choose trace to follow:", shMenu::kNoHelp);
    } else {
        trace_menu = I->newMenu ("View traces:", shMenu::kNoHelp | shMenu::kNoPick);
    }
    char let = 'a';
    int found = 0;
    trn::WarpTrace *trace = trn::wmon->first ();
    while (trace) {
        if (trace->level == Hero.cr ()->mLevel and
            trace->time + 20 * FULLTURN >= Clock)
        {
            if (disk->is (obj::infected) and !RNG (3))
                continue;
            ...
            snprintf (desc, SHBUFLEN, "%s transported to a %s location.",
                trace->who,
                dist < 60 ? "neighboring" :
                dist < 120 ? "nearby" : "distant");
            trace_menu->addPtrItem (let++, desc, trace);
            ++found;
        }
        trace = trn::wmon->next (trace);
    }
    ...
    if (trace_menu->getPtrResult ((const void **) &trace)) {
        int x = -1, y = -1;
        if (Hero.cr ()->is (kConfused)) {
            x = trace->src_x;
            y = trace->src_y;
...
{   /* Transport may be completely blocked. */
    ...
    while (tries--) {
        ...
        if (-1 == res) {
            ...
            trn::wmon->trace (this, oldX, oldY, mX, mY);
            if (isHero ())
...
doAntiVirus (shObject *computer, shObject *disk)
{
    ...
    bool unrestricted = disk->is (obj::cracked) and disk->is (obj::known_cracked);
    int howmany = RNG (1, 3) + disk->isOptimized () * 2;
