...
shInterface::saveOptions ()
{
    ...
    if (config == NULL) {
        ...
        p ("Changes to options are transient.");
        pause ();
        return;
    }
...
shInterface::keymapChoice ()
{
    ...
    DIR *dir = opendir (KEYMAPDIR);
    if (dir == NULL) {
        p ("Error opening directory '%s'.", KEYMAPDIR);
        return;
    }

    char *prime_map = GetBuf ();
    char *title = GetBuf ();
    ...
    while ((data = readdir (dir))) {
        ...
        char *file_name = GetBuf ();
        strcpy (file_name, &data->d_name[0]);
        /* Try to open file beforehand. */
        char path[PRIME_PATH_LENGTH];
        snprintf (&path[0], sizeof (path)-1, "%s/%s", KEYMAPDIR, data->d_name);
        ...
        /* Mark current. */
        if (strcmp (Flags.mKeySet, file_name) == 0)
            strcat (title, " (CURRENT)");
        /* Save default. */
        if (strstr (title, "(DEFAULT)") != NULL)
            strcpy (prime_map, file_name);
        /* Add option and close file. */
        keymaps->addPtrItem (letter++, &title[0], file_name);
        fclose (candidate);
    }
    char *fname = NULL;
    (void) keymaps->getPtrResult ((const void **) &fname);
    if (fname != NULL)
        readKeybindings (fname);
    else if (Flags.mKeySet[0] == '\0')
        readKeybindings (prime_map);
    delete keymaps;
}
